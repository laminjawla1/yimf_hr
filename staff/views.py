...
def dashboard(request):
    ...
    full_time_staffs = len(Employee.objects.filter(
        employment_status="Full Time").all())
    part_time_staffs = len(Employee.objects.filter(
        employment_status="Part Time").all())
    probation = len(Employee.objects.filter(
        employment_status="Probation").all())
    ...
    on_suspension = len(Employee.objects.filter(
        employment_status="On Suspension").all())
    ...
    temporary = len(Employee.objects.filter(
        employment_status="Temporary").all())
    contract = len(Employee.objects.filter(employment_status="Contract").all())
    independent_contract = len(Employee.objects.filter(
        employment_status="Independent Contract").all())
...
def staffs(request):
    return render(request, "staff/staffs.html", {
        'staffs': Employee.objects.all(), 'current_page': 'staffs'
    })
